  ...
  if (!reqs?.length) { reqList.innerHTML = '<div class="empty-state"><div class="empty-state-icon">📋</div><p>No requests yet. <a href="#" data-page="new-request" style="color:var(--accent)">Create one →</a></p></div>'; }
    ...
    <div class="request-card" style="margin-bottom:0.75rem">
      ...
      <div style="display:flex;gap:1rem;font-size:0.8rem;color:var(--text3)">
        ...
        ${r.final_price ? `<span style="color:var(--accent)">$${r.final_price}</span>` : r.suggested_price ? `<span>Est. $${r.suggested_price}</span>` : ''}
      </div>
    </div>`).join('');
  ...
  list.innerHTML = data.map(p => `
    <div class="project-card" onclick="loadProjectDetail('${p.id}')">
      ...
      <div class="project-card-meta">
        ...
        ${badge(p.status)}
        ${p.price ? `<span style="color:var(--accent);font-family:var(--font-mono);font-size:0.85rem">$${p.price}</span>` : ''}
      </div>
    </div>`).join('');
}
      ...
      <div class="project-detail-header">
        <div>
          ...
          <div class="project-meta-row">
            ...
            ${proj.price ? `<span class="project-meta-item" style="color:var(--accent)">💰 $${proj.price} ${proj.is_paid ? badge('done') : badge('pending')}</span>` : ''}
          </div>
        </div>
      </div>
      <div class="project-detail-grid">
        ...
        <div class="project-sidebar">
          <div class="sidebar-card">
            ...
            <div class="sidebar-row"><span>Price</span><span>${proj.price ? '$'+proj.price : 'TBD'}</span></div>
...
async function loadAdminOverview() {
  ...
  document.getElementById('adm-total-revenue').textContent = `$${revenue.toFixed(2)}`;
    ...
    <div class="request-card">
      ...
      <div class="request-card-actions">
        ${r.status === 'pending' ? `
          ...
          <input class="price-input" id="price-${r.id}" type="number" placeholder="Price $" value="${r.final_price||r.suggested_price||''}" />
          <span style="font-size:0.8rem;color:var(--text3)">Suggested: $${r.suggested_price||0}</span>
        ` : r.status === 'approved' ? `
          <span style="color:var(--green);font-size:0.85rem">✓ Approved</span>
          ${r.final_price ? `<span style="color:var(--accent);font-family:var(--font-mono)">$${r.final_price}</span>` : ''}
...
window.approveRequest = async function(reqId) {
  ...
  if (req) {
    ...
    await sendNotification(req.user_id, 'Request Approved', `Your request "${req.title}" was approved!${price ? ` Price: $${price}` : ''}`, 'success', 'project', proj?.id);
...
async function loadAdminProjects() {
  ...
  el.innerHTML = `<table class="data-table">
    ...
    <tbody>${data.map(p => `<tr>
      ...
      <td style="font-family:var(--font-mono)">${p.price ? '$'+p.price : '—'}</td>
...
window.markProjectPaid = async function(projId) {
  ...
  if (proj) {
    await sendNotification(proj.user_id, 'Payment Confirmed', `Payment of $${proj.price} confirmed for "${proj.title}".`, 'success', 'project', projId);
...
async function loadAdminAccounting() {
  ...
  document.getElementById('acc-total').textContent = `$${total.toFixed(2)}`;
  document.getElementById('acc-month').textContent = `$${monthTotal.toFixed(2)}`;
  ...
  el.innerHTML = `<table class="data-table">
    ...
    <tbody>${payments.map(p => `<tr>
      ...
      <td style="color:var(--text2)">${escHtml(p.users?.full_name||'—')}</td>
      <td style="color:var(--accent);font-family:var(--font-mono)">$${Number(p.amount).toFixed(2)}</td>
