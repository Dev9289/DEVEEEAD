...
async function onLogin(user) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (error) {
      console.warn(`Profile fetch attempt ${attempt + 1} failed:`, error.message, error.details);
      // If error 406, it's likely the join failing. Try without join as fallback.
      if (error.code === 'PGRST107' || error.status === 406) {
        const { data: simpleData } = await sb.from('users').select('*').eq('id', user.id).single();
        if (simpleData) {
          profile = simpleData;
          console.log('Fetched profile without roles join.');
          break;
        }
      }
    }
  ...
  if (!profile) {
    ...
    try {
      ...
      const { data: roleData, error: roleError } = await sb.from('roles').select('id').eq('name', 'user').single();
      if (roleError) console.error('Error fetching default role:', roleError);
      ...
      const { error: upsertError } = await sb.from('users').upsert(profileData, { onConflict: 'id' });
  ...
  isAdmin = profile?.roles?.name === 'admin' || profile?.role_id === (await sb.from('roles').select('id').eq('name', 'admin').single()).data?.id;
...
document.getElementById('register-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    // If user is immediately available (email confirmation disabled), create profile & login
    const userId = data.user?.id;
    if (userId) {
      console.log('User ID created:', userId);
      // We rely on the Supabase trigger to create the profile row.
      // If the session exists immediately (no email confirmation), we navigate to dashboard.
      if (data.session) {
        toast('Account created! Welcome to DEVLO.', 'success');
        navigate('dashboard');
        return;
      }
    }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function hideLoader() { if (ls) ls.classList.add('hidden'); }

  document.getElementById('app').style.display = 'block';
  navigate('home');

  // Hard safety: loader always gone after 2s no matter what
  const safetyTimer = setTimeout(hideLoader, 2000);

  // If Supabase failed to load (offline / file://) bail gracefully
